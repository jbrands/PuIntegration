...
void plotROC_eta0to2(int scenario, const char* input1, const char* input2, const char* output){
  ...
  /*for (vector<string>::iterator it = legendNames.begin(); it != legendNames.end(); ++it)
    std::cout << ' ' << *it;
    std::cout << '\n';*/
  ...
  for(int i=0; i<pt_new.size(); i++){
    if(pt_new.at(i) > 20 && pt_new.at(i) < 30 && abs(eta_new.at(i)) < 2. && mvaOut_new.at(i) != -2){
  ...
  for(int i=0; i<pt_new.size(); i++){
    if(pt_new.at(i) > 30 && pt_new.at(i) < 50 && abs(eta_new.at(i)) < 2. && mvaOut_new.at(i) != -2){
  ...
  for(int i=0; i<pt_old.size(); i++){
    if(pt_old.at(i) > 20 && pt_old.at(i) < 30 && abs(eta_old.at(i)) < 2. && mvaOut_old.at(i) != -2){
  ...
  for(int i=0; i<pt_old.size(); i++){
    if(pt_old.at(i) > 30 && pt_old.at(i) < 50 && abs(eta_old.at(i)) < 2. && mvaOut_old.at(i) != -2){
  ...
  TCanvas *c1 = new TCanvas("c1", "eta0to2", 200,10,700,500);
  c1->cd();
  c1->SetFillColor(0);
  ...
  TH1F *hr = c1->DrawFrame(0.75,0.05,1.,1.);
  //hr->SetTitle("0<|#eta|<2, 20<p_T<30GeV");
  ...
  outstream_eps << output << "_ROC_pt20to30_eta0to2.eps";
  outstream_svg << output << "_ROC_pt20to30_eta0to2.svg";
  ...
  TCanvas *c2 = new TCanvas("c2", "eta0to2", 200,10,700,500);
  c2->cd();
  c2->SetFillColor(0);

  c2->SetLogy(1);
  //c1->SetLogx(1);                                                                                   
  TH1F *hr2 = c2->DrawFrame(0.75,0.05,1.,1.);
  //hr2->SetTitle("0<|#eta|<2, 30<p_T<50GeV");
  ...
  outstream_eps << output << "_ROC_pt30to50_eta0to2.eps";
  outstream_svg << output << "_ROC_pt30to50_eta0to2.svg";
