...
void plotROC_eta3to5(int scenario, const char* input1, const char* input2, const char* output){
  ...
  if(file_old.good()){
	...
	int pos5 = linestring_old.substr(pos1+pos2+pos3+pos4+7).find("\n");
  ...
  for(int i=0; i<pt_new.size(); i++){
    if(pt_new.at(i) > 20 && pt_new.at(i) < 30 && abs(eta_new.at(i)) > 3. && abs(eta_new.at(i)) < 5. && mvaOut_new.at(i) != -2){
  ...
  for(int i=0; i<pt_new.size(); i++){
    if(pt_new.at(i) > 30 && pt_new.at(i) < 50 && abs(eta_new.at(i)) > 3. && abs(eta_new.at(i)) < 5. && mvaOut_new.at(i) != -2){
  ...
  for(int i=0; i<pt_old.size(); i++){
    if(pt_old.at(i) > 20 && pt_old.at(i) < 30 && abs(eta_old.at(i)) > 3. && abs(eta_old.at(i)) < 5. && mvaOut_old.at(i) != -2){
  ...
  for(int i=0; i<pt_old.size(); i++){
    if(pt_old.at(i) > 30 && pt_old.at(i) < 50 && abs(eta_old.at(i)) > 3. && abs(eta_old.at(i)) < 5. && mvaOut_old.at(i) != -2){
  ...
  TCanvas *c1 = new TCanvas("c1", "eta3to5", 200,10,700,500);
  c1->cd();
  c1->SetFillColor(0);
  ...
  TH1F *hr = c1->DrawFrame(0.75,0.3,1.,1.2);
  //hr->SetTitle("3<|#eta|<5, 20<p_T<30GeV");
  ...
  outstream_eps << output << "_ROC_pt20to30_eta3to5.eps";
  outstream_svg << output << "_ROC_pt20to30_eta3to5.svg";
  ...
  TCanvas *c2 = new TCanvas("c2", "eta3to5", 200,10,700,500);
  c2->cd();
  c2->SetFillColor(0);

  c2->SetLogy(1);
  //c1->SetLogx(1);                                                                                   
  TH1F *hr2 = c2->DrawFrame(0.75,0.3,1.,1.2);
  hr2->SetTitle("3<|#eta|<5, 30<p_T<50GeV");
  ...
  outstream_eps << output << "_ROC_pt30to50_eta3to5.eps";
  outstream_svg << output << "_ROC_pt30to50_eta3to5.svg";
